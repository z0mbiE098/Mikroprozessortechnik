...
static const unsigned int time_periods[10] = {			//Über BCD-Schalter gesteuerte Zeitintervalle
...
void initSegment7(void){								// Initilialisiert das 7-Segment-Display als Ausgang
    IODIR0 |= SEGMENT7_MASK;            // P0.18-P0.24 als Ausgang. Auch hier nur die Bits von 7Segment überschreiben, keine anderen!
...
unsigned int readBCDInput(void){				// Liest den Wert eines BCD-Codierschalters an P.010 - P0.13 ein.
    return (IOPIN0 >> 10) & 0xF;				// schieb die bits um 10 Stellen nach rechts, damit steht P0.10 an Bit0. Mit & 0xF bleiben die untere 4 Bits übrig.
}
...
void updateSegment7(unsigned int value){		// Zeigt eine Zahl 0-9 auf das Segment
    if(value > BCD_MAX) value = BCD_MAX;		// Array Überlauf schutz, Falls value>9, dann auf 9 beschränken
...
unsigned int readSwitchState(void){					// liest den Status der Schalter S1, S2(Port 0) und S3(Port 1)
    unsigned int s = 0;
    if ((IOPIN0 & (1U << 16)) == 0U) s |= 1U; // IoPin0& eine Zahl, wobei nur das 16.Bit 1 ist. Falls Taste gedrückt wird, steht an 16. Stelle eine 0 und das Ergebnis ist dann 0. 
																							// s |= 1U sagt an der 0te Stelle von s eine 1 schreiben. Damit S1 und S2 gleichzeitig funktionieren können, bracuhen wir |=
    if ((IOPIN0 & (1U << 17)) == 0U) s |= 2U;  
    if ((IOPIN1 & (1U << 25)) == 0U) s |= 4U; 
    return s;
}

void initTimer(void){										// Timer0 konfigurieren für periodische Interrupts
    ...
    T0MR0 = 1000;                      	// Default: 1000 ms = 1s. Nach jedem Sekunde wird der Controller den (aktuellen Zählerstand) T0TC mit T0MR0 vergleichen. 
    T0MCR = 0x03;                      	// Bei einem Match --> Interrupt auslösen und Reset

    VICVectAddr4 = (unsigned long)T0isr; // Adresse der Funktion für Slot 4 festlegen
    VICVectCntl4 = 0x20 | 4;             // Slot enable + Timer0 Kanal 4 zuweisen
    VICIntEnable = (1 << 4);             // Interrupt enable global für Timer0 einschalten
...
void T0isr(void) __irq{
    ...
    if (T0MR0 != time_periods[bcd_in]) {			// Gucken ob die eingestellte Zeit in T0MR0 immer noch dieselbe ist, falls nicht, dann kurz aktualisiern
          T0TCR = 0x00;                        // Anhalten, damit keine neue Signale auftauchen können
					T0MR0 = time_periods[bcd_in];     		// Timer aktualisieren.  
			    T0MCR = 0x03;													// Wenn Ziel erreicht, dann Interrupt auslösen und sich auf NULL Setzen.
					T0TCR = 0x02;                        // reset. 
	 ...
	 if ((sw & 1U) != 0U){              // Logik Für Taste S1: Prüfe ob Bit 0 in sw gestezt ist. Ist das unterste Bit eine 1? falls ja setzt alles auf NULL
        updateLED(0);									// Alle LEDs sind aus
        IOCLR0 = SEGMENT7_MASK; 			// 7-Segment auch komplett aus
        led_pos = 0;									// Zähler für Lauflicht auf 0
        segment7_digit = 0;						// Zähler für Segment auf 0
    } else {

        if ((sw & 2U) != 0U) {				// S2 gedrückt? 

...
void initBCD(void){
	...
	IOCLR0 = SEGMENT_MASK;								// BCD Anzeige initial l�schen
}

unsigned int readBCDInput(void){
	return (IOPIN0 >> 10) & 0xF;					// IOPIN0 enth�lt logischen Zustand aller Pins auf Port 0. Dieser Zustand wird dann um 10 Stellen nach rechts geshiftet,
...
void updateLED(unsigned int pattern){   
	...
	IOSET1 = (pattern << 16);							// Eingegebene LED wird um 16 Stellen nach links geschoben, um die LEds die bin�re Darstellung des BCDs Wert zu geben
}

void updateBCD(unsigned int value){
	if(value > BCD_MAX) value = BCD_MAX;	// Falls eingegebene Value >9 dann bleibt es bei 9
	
	IOCLR0 = SEGMENT_MASK;								// L�scht alle Segmente von 7-Segment Anzeige

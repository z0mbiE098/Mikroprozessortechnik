...
;********************************************************************
				...
				LDR R0, =X		; Load Register: Die Adresse von vollst�ndiges 32 Bit Wort wird geladen
				BL Berechnung   	; 
...
AtoI_Loop
				LDRB    R1, [R0], #1    ; Load Register Byte. Zeochen aus R0 wird geladen und Zeiger zeigt mit #1 direkt auf die n�chste Stelle
                ...
                MOV     R4, R2, LSL #3      ; Ersten teil der Multiplikation.
											; Die zahl die bisher in R2 steht muss auf Zehnerstelle r�cken, um platz f�r die n�chste Zahl zu machen.
                ADD     R4, R4, R2, LSL #1  ; Add nimmt das Ergebnis von R4 und addiert die R2*2 dazu.
											; R4  = Die bisherige Zahl(multipliziert mit 10)
                ADD     R2, R4, R1          ; Jetzt m�ssen wir die neue Zahl bilden. (Alte Zahl in R4(60) + neue Zahl in R1(5) = "65___"
                B       AtoI_Loop			; Zur�ck zum Anfang, wo es weitergeht
...
Formel
				...
				MOV        R3, R4, LSR #1   ; Um die gr��e Zahl mit genauigkeit in einem register zu schreiben machen wir ein Right shift.(siehe "n" auf der Siete 96)
											; Also (X^2/9) wird grob gesch�tzt und im Register R3 gespeichert.
				...
				BX		LR					; Wieder zu R�cksprungadressen springen
...
uItoBCD
		...
		LDR  R5, =DIV_10					; R5 bekommt magische Nummer f�r Division durch 10
		MOV  R6, #10						; R6 bekommt die Konstante Zahl 10 f�r Modulo-Operation
...
uItoBCD_Loop
        ...
        MOV R3, R7, LSR#3  					; Genauere Ergebis in 32 Bit, R7 muss noch 3 bits geshiftet werden(siehe "n" Seite 96)
											; F�r 123 berechnen wir: q = 123/10 = 12(kommt in R3)
		...
		MOV R0, R3          				; R3 enth�lt die Quotient, das wird in R0 als Eingabe geschickt und die n�chste Iteration beginnt
		ADD R2, R2, #4       				; bei jeder iteration wird einmla mehr um 4 geshiftet, damit die n�chste Dezimalzahl richtig positioniert wird

		B    uItoBCD_Loop					; Sprint zur�ck zum Anfang der Schleife

uItoBCD_Done
        MOV  R0, R1          				; Fertige BCD Ergnis wird in R0 gemoved 
        LDMFD SP! ,{R1-R8, LR}				; Registern wieder freimachen
        BX   LR								; Zur R�cksprungadresse springen
				
;------------------------Aufgabe 4-----------------------------------

Berechnung
                STMFD SP!, {LR}         ; R�cksprungadresse sichern
